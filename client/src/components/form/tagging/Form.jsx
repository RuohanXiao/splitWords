...
import CollectionImportForm from './ImportForm';
// import CollectionCreateForm from './CustomizedForm';
...
export default class UForm extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      address: '',
      timeRange: '',
      importVisible: false, //导入窗口隐藏
      createVisible: false, //新建窗口隐藏
      dataSource: [],
      count: data.length,
      selectedRowKeys: [],
      tableRowKey: 0,
  ...
  render() {
    const { userName, address, timeRange, dataSource, visible, isUpdate,importVisible, loading } = this.state;
    const questiontxt = () => {
      return (
        <p>
        ...
        <div className='formBody'>
          ...
          <Row gutter={16}>
            ...
            <div className='btnOperaCenter'>
              ...
              <Button type="info" onClick={this.ImportItem} style={{ margin: '10px', backgroundColor: "#87d068", color: "white" }}>导入文本</Button>
              <Button type="info" onClick={this.CreateItem} style={{ margin: '10px', backgroundColor: "#87d068", color: "white" }}>导出文本</Button>
              <Button type="info" onClick={this.CreateItem} style={{ margin: '10px', backgroundColor: "#87d068", color: "white" }}>分发任务</Button>
              <Button type="info" onClick={this.CreateItem} style={{ margin: '10px', backgroundColor: "#87d068", color: "white" }}>创建分类</Button>
             
            </div>
          </Row>
          <FormTable onClick = { () => this.editClick(this.record.key)}
          ...
          {isUpdate ?
            ...
            />}
          {importVisible?
          <CollectionImportForm flag={{ importVisible }} style={{ textAlign: 'center', margin: '5%' }} ref={this.saveFormRef} visible={visible} onCancel={this.handleCancel} onCreate={this.handleUpdate} title="情感标注" okText="保存当前标注" cancelText="放弃当前标注"/>:''
        }  
        </div>
      </div>
    )
